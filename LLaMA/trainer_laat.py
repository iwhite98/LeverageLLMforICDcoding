...
class Trainer:
	...
	def _set_signature_columns_if_needed(self):
		if self._signature_columns is None:
			...
			self._signature_columns += list(set(["label", "label_ids", "code_labels", 'input_ids_ref', 'labels_ref', 'input_ids_att', 'attention_mask_ref', 'attention_mask_att'] + self.label_names)) ##########
		...
		# Load adapters following PR # 24096
			...
			if hasattr(model, "active_adapter") and hasattr(model, "load_adapter"):
				if os.path.exists(resume_from_checkpoint):
					...
					model.model.model.ref_linear.load_state_dict(torch.load(resume_from_checkpoint + '/kg_model.bin'))
					model.label_attention.load_state_dict(torch.load(resume_from_checkpoint + '/laat_model.bin'))
				else:
					logger.warning(
	...
	def _save_checkpoint(self, model, trial, metrics=None):
		...
		######################### kg save
		torch.save(model.model.model.ref_linear.state_dict(), output_dir + '/kg_model.bin')
		######################### laat save
		torch.save(model.label_attention.state_dict(), output_dir + '/laat_model.bin')
	...
	def compute_loss(self, model, inputs, return_outputs=False):
		...
		label_view = shift_labels.view(-1)
		...
		if 'labels_ref' in inputs:
			labels_ref = inputs.pop('labels_ref')
			shift_labels_ref = labels_ref[..., 1:].contiguous()
			label_ref_view = shift_labels_ref.view(-1)
		
		code_labels = inputs.pop("code_labels")

		#input_ids_ref = inputs.pop('input_ids_ref')
		#attention_mask_ref = inputs.pop('attention_mask_ref')
...
#			  loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]
		...
		### for decoder + laat + kg
		if 'output_att_ref' in outputs.keys():
			output_att = outputs["output_att"]
			output_dec = outputs["logits"][..., :-1, :].contiguous()
			output_att_ref = outputs["output_att_ref"]
			output_dec_ref = outputs["logits_ref"][..., :-1, :].contiguous()
			loss_att = nn.functional.binary_cross_entropy_with_logits(output_att, code_labels)
			loss_att_ref = nn.functional.binary_cross_entropy_with_logits(output_att_ref, code_labels)
			#loss_dec = nn.functional.cross_entropy(output_dec.view(-1, 32000), label_view)
			#loss_dec_ref = nn.functional.cross_entropy(output_dec_ref.view(-1, 32000), label_ref_view)
			import torch
			loss_cr = nn.functional.mse_loss(torch.sigmoid(output_att), torch.sigmoid(output_att_ref))
			loss = 0.7 * loss_att + 0.3 * loss_att_ref + 0.3 * loss_cr
			return (loss, outputs) if return_outputs else loss
		elif 'output_att' not in outputs.keys():
			if 'logits_ref' in outputs.keys():
				output_dec = outputs['logits'][..., :-1, :].contiguous()
				output_dec_ref = outputs['logits_ref'][..., :-1, :].contiguous()

				loss_dec = nn.functional.cross_entropy(output_dec.view(-1, 32000), label_view)
				loss_dec_ref = nn.functional.cross_entropy(output_dec_ref.view(-1, 32000), label_ref_view)
				loss = 0.7 * loss_dec + 0.3 * loss_dec_ref
				return (loss, outputs) if return_outputs else loss
			else:
				output_dec = outputs['logits'][..., :-1, :].contiguous()
				loss = nn.functional.cross_entropy(output_dec.view(-1, 32000), label_view)
				return (loss, outputs) if return_outputs else loss
		else:
			output_att = outputs["output_att"]
			output_dec = outputs["logits"][..., :-1, :].contiguous()
			loss = nn.functional.binary_cross_entropy_with_logits(output_att, code_labels)
			return (loss, outputs) if return_outputs else loss
